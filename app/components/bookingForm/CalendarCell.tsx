...
export default function  CalendarCell({state,date,currentMonth,isUnavailable} : {
    state : CalendarState, date : CalendarDate, currentMonth : CalendarDate, isUnavailable? : boolean
}) {
    let ref = useRef(null);
    let {
      cellProps,
    ...
    const finalyIsDisabled = isDisabled || isUnavailable
    ...
    return (
       <td {...cellProps} className={`py-0.5 px-0.5 relative ${isFocusVisible ? "z-10" : "z-0"}`}>
          <div {...mergeProps(buttonProps, focusProps)} ref={ref} hidden={isOutsideOfMonth} className="size-10 sm:size-12 outline-none group rounded-md">
              <div className={cn("size-full rounded-sm flex items-center justify-center text-sm font-semibold",finalyIsDisabled ? 'text-muted-foreground cursor-not-allowed' : "", isSelected ? 'bg-primary text-white' : '',
